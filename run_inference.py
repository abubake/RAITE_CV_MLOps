...
def run_inference():
    ...
    transform = transforms.Compose(
        [transforms.ToPILImage(), transforms.Resize((200, 200)), transforms.ToTensor()]
    )

    # dir_path = "data/archive/test_sets/drone/t2_autonomyPark150/images"
    dir_path = "/home/eherrin@ad.ufl.edu/code/gitlab_dev/raiteclassify/data/archive/test_sets/special_cases/attack_on_autonomyPark/images"
    ...
    for image_name in all_images:
        ...
        original_images.append(
            image.copy()
        )  # Store a copy of the original image for visualization
    ...
    with torch.no_grad():
        predictions = model(
            test_images[start_index:end_index]
        )  # or w/o list, test_images[:N]
    ...
    for i, prediction in enumerate(predictions):
        ...
        for box, score in zip(boxes, scores):
            ...
            if score > 0.1:  # Only consider predictions with confidence score > 0.5
                ...
                cv2.rectangle(
                    original_images[i + start_index], (x1, y1), (x2, y2), (0, 255, 0), 2
                )

                # Put a label with the score
                label_text = f"Score: {score:.2f}"
                cv2.putText(
                    original_images[i + start_index],
                    label_text,
                    (x1, y1 - 10),
        ...
        for objectID, centroid in centerTracker.objects.items():
            ...
            cv2.circle(
                original_images[i + start_index], (int(cx), int(cy)), 7, (0, 0, 255), -1
            )  # Draw the centerpoint in blue
        ...
        attacks = anomalyDetector.detect_attack(
            frame_index=i + start_index,
            frame=original_images[i + start_index],
            detections=rescaled_boxes,
            centroids=centroids,
        )
